...
import { JWT_SECRET_ACCESS_KEY, JWT_EXPIRES_IN, NODE_ENV } from "../../constants/env.js";
...
const signup = async (req, res) => {
  ...
  if (fullname.length < 3) {
    return sendResponse(res, 400, "error", "Full name should be at least 3 letters long", null);
  }
  ...
  try {
    ...
    if (subscribeUser) {
      if (await User.exists({ "personal_info.email": subscribeUser._id })) {
        return sendResponse(res, 400, "error", "Email is already registered", null);
      }
