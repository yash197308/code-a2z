...
const invitationToCollaborate = async (req, res) => {
  try {
    ...
    if (!project) return sendResponse(res, 404, 'error', 'Project not found', null);

    const author = project.author;
    if (!author) return sendResponse(res, 404, 'error', 'Project author not found', null);

    if (String(user._id) === String(author._id)) {
      return sendResponse(res, 400, 'error', 'You cannot invite yourself to collaborate on your own project', null);
    }

    const subscriber = await Subscriber.findOne({ _id: author.personal_info.email }).select('email');
    ...
    try {
      ...
      return sendResponse(res, 500, 'error', 'Failed to send invitation email', null);
    }
    ...
    if (emailSent) {
      ...
      return sendResponse(res, 200, 'success', 'Invitation sent successfully', collaborationData);
    }
  } catch (err) {
    return sendResponse(res, 500, 'error', err.message || 'Failed to send invitation', null);
  }
};
